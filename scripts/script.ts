...
const svgWidth:number = 445 - margin.left - margin.right;
const svgHeight:number = 445 - margin.top - margin.bottom;
...
d3.json<any>("https://cdn.freecodecamp.org/testable-projects-fcc/data/tree_map/movie-data.json", 
{
  method: "GET",
  mode: "cors"
})
  .then(data => {
    ...
    const color:d3.ScaleOrdinal<string, any, never> = d3.scaleOrdinal()
      .domain(categories)
    ...
    const legendWidth:number = (svgWidth + margin.left + margin.right) + 50;
    const legend = legendContainer.append("svg")
      .attr("id", "legend")
      .attr("width", legendWidth)
    ;
    const legendOffset:number = 10;
    const legendRectSize:number = 15;
    const legendHSpacing:number = 150;
    const legendVSpacing:number = legendOffset;
    const legendTextXOffset:number = legendRectSize + 3;
    const legendTextYOffset:number = legendOffset + 2;
    const legendElemsPerRow:number = Math.floor(legendWidth / legendHSpacing);
